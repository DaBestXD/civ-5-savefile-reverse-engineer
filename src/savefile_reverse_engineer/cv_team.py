...
from ._cv_team_hashes import TEAM_HASH_NAMES
from .cv_plot_types import HashedType, RouteType
...
def _resolve_hash(hash_value: int) -> HashedType:
    return HashedType(hash_value=hash_value, name=TEAM_HASH_NAMES.get(hash_value))
...
def _read_project_art(
    ...
    count_offset = reader.offset
    count = reader.u32("project_art_types.count")
    if count != _PROJECT_COUNT:
        reader.fail(
            f"project_art_types.count is {count}, expected {_PROJECT_COUNT}",
            offset=count_offset,
        )
    counts_by_hash = {
...
def _read_technologies(
    ...
    count_offset = reader.offset
    count = reader.u32("technologies.count")
    if count != _TECHNOLOGY_COUNT:
        reader.fail(
            f"technologies.count is {count}, expected {_TECHNOLOGY_COUNT}",
            offset=count_offset,
        )
    hashes = tuple(
...
def _read_improvement_yield_array(
    ...
    return _read_hashed_values(
        ...
        read_value=lambda value_field: _read_yield_changes(reader, value_field),
        field=field,
    )


def _read_revealed_resources(reader: _Reader) -> tuple[HashedType, ...]:
    count = reader.u32("revealed_resources.count")
