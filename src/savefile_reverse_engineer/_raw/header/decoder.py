...
def _read_count(
    ...
    *,
    ...
    count = read_u32_count(reader, field, expected=expected)
...
def _read_vector[ValueT](
    ...
    *,
    ...
    return tuple(read_value(f"{field}[{index}]") for index in range(count))
...
def _read_i32_vector(
    ...
    return _read_vector(
        reader, field, reader.i32, minimum_item_size=4, expected=expected
    )
...
def _read_bool_vector(
    ...
    return _read_vector(
        reader, field, reader.read_bool, minimum_item_size=1, expected=expected
    )
...
def _read_string_vector(
    ...
    return _read_vector(
        reader, field, reader.read_utf8, minimum_item_size=4, expected=expected
    )
...
def _read_quick_header(
    ...
    # TODO(decoding): Replace each preserved quick-header bridge span with named
    # fields on the raw QuickHeader model when its layout is confirmed.
