...
from .binary_reader import LittleEndianReader
...
def read_free_list_header(reader: LittleEndianReader, field: str) -> FreeListHeader:
    ...
    next_free_indices = tuple(
        reader.i32(f"{field}.next_free_indices[{index}]") for index in range(slot_count)
    )
