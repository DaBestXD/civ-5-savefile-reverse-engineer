"""Shared bounded reader for private little-endian raw decoders."""
...
def read_u32_count(
    reader: LittleEndianReader, field: str, *, expected: int | None = None
) -> int:
    """Read a vector count and optionally require one exact value."""
    count_field = f"{field}.count"
    count_offset = reader.offset
    count = reader.u32(count_field)
    if expected is not None and count != expected:
        reader.fail(
            f"{count_field} is {count}, expected {expected}",
            offset=count_offset,
            field=count_field,
        )
    return count
...
def enum_value[EnumType: IntEnum](
    ...
    *,
    ...
    """Convert a raw integer to an enum or raise through the active reader."""
    try:
        return enum_type(raw_value)
    except ValueError:
        reader.fail(
            f"unsupported {value_name} value {raw_value}",
            offset=offset,
            field=field,
        )
