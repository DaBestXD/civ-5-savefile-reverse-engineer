...
from .._catalogue.plots import (
    BUILD_HASH_NAMES,
    FEATURE_HASH_NAMES,
    IMPROVEMENT_HASH_NAMES,
    RESOURCE_HASH_NAMES,
)
from .._shared.binary_reader import LittleEndianReader, enum_value
...
def _read_hash(reader: _Reader, names: Mapping[int, str], *, field: str) -> HashedType:
    return resolve_hashed_type(reader.u32(field), names)
...
def _read_build_progress(
    ...
    for build_index in range(inner_count):
        ...
        entries.append(
            BuildProgress(
                build=resolve_hashed_type(hash_value, BUILD_HASH_NAMES),
                progress=progress,
            )
        )
